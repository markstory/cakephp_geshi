...
class GeshiHelperExpandedTest extends CakeTestCase {
...
/**
 * Run each of the base five (5) tests multiple times by setting the same settings
 * in multiple ways.
 */
 ...
 * @return void
 */
	private function __testHighlight() {
 ...
 * @return void
 */
	private function __testPlainTextButton() {
		// Simple one code block
		$text = '<p>This is some text</p><pre lang="php"><?php echo $foo = "foo"; ?></pre><p>More text</p>';
		$result = $this->geshi->highlight($text);
		$expected = array(
	...
	private function __testNoTagReplacement() {
		// Simple one code block
		$this->geshi->showPlainTextButton = false;
	...
	private function __testHighlightText() {
	...
	private function __testHighlightAsTable() {
		$text = <<<CODE
<?php
echo 'test';
echo 1 + 1;
